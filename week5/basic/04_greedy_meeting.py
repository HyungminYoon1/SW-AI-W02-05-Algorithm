...
def select_meetings(meetings):
    ...
    for i in range(1, len(sorted_meetings)):
        former_meeting_end_time = selected[-1][1] # 마지막으로 선택된 회의의 종료시간
        current_meeting_start_time = sorted_meetings[i][0]

        if former_meeting_end_time <= current_meeting_start_time:
            selected.append(sorted_meetings[i])
